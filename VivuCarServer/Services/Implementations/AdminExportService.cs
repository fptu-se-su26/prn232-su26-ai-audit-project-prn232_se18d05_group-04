...
public class AdminExportService(IAdminExportRepository repository,IAdminExportFileStore store):IAdminExportService
{
    ...
    private static byte[] Csv(AdminReportPreviewResponse p){var b=new StringBuilder();b.AppendLine(string.Join(',',p.Headers.Select(E)));foreach(var row in p.Rows)b.AppendLine(string.Join(',',row.Select(E)));var encoding=new UTF8Encoding(true);var preamble=encoding.GetPreamble();var content=encoding.GetBytes(b.ToString());var result=new byte[preamble.Length+content.Length];Buffer.BlockCopy(preamble,0,result,0,preamble.Length);Buffer.BlockCopy(content,0,result,preamble.Length,content.Length);return result;static string E(string v)=>$"\"{v.Replace("\"","\"\"")}\"";}
