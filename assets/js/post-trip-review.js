(function () {
  ...
  function render() {
    document.getElementById("breadcrumbMount").innerHTML = window.VivuCarLayout.renderBreadcrumb([{ label: "Đơn thuê", href: "my-bookings.html" }, { label: "Đánh giá" }]);
    if (!booking || !car) return root.innerHTML = U.renderEmptyState({ title: "Không tìm thấy chuyến đi", href: "my-bookings.html", action: "Về đơn thuê" });
    if (booking.status !== "completed") return root.innerHTML = U.renderEmptyState({ title: "Chưa thể đánh giá", text: "Chỉ đánh giá khi booking.status = completed.", href: `booking-detail.html?bookingId=${booking.id}`, action: "Xem đơn" });
